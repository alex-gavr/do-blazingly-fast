...
import { TWebVitalsStaticVsEdge, webVitalsStaticVsEdge } from '@db/schema';
...
export const post: APIRoute = async ({ request }) => {
  ...
  const webVitalsData = (await request.json()) as TWebVitalsStaticVsEdge;
  ...
  try {
    const res = await db.insert(webVitalsStaticVsEdge).values({
      id: webVitalsData.id,
      geo: webVitalsData.geo,
      pathname: webVitalsData.pathname,
      offer: webVitalsData.offer,
      output: webVitalsData.output,
